...
public class WebSocketChannelInterceptor implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel messageChannel){
        ...
        if (StompCommand.CONNECT.equals(accessor.getCommand())) {
            ...
            //System.out.println("认证用户:" + user.toString() + " 页面传递令牌" + httpSession.getId());
    ...
    public void postSend(Message<?> message, MessageChannel messageChannel, boolean b){
        ...
        log.info("postSend 中获取httpSession key：" + accessor.getSessionId());
        ...
        switch (accessor.getCommand()) {
            // 首次连接
            case CONNECT:
                log.info("httpSession key：" + accessor.getSessionId() + " 首次连接");
                String sessionId = accessor.getSessionId();
                globalProperties.addTransactionAndSession(sessionId, sessionId);
                break;
            // 连接中
            case CONNECTED:
                break;
            // 下线
            case DISCONNECT:
                log.info("httpSession key：" + accessor.getSessionId() + " 下线");
                if (globalProperties.getSessionIds().contains(accessor.getSessionId())) {
                    globalProperties.removeTransactionAndSession(accessor.getSessionId());
                }
                break;
            default:
                break;
